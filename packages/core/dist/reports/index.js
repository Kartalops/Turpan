...
export function generateMarkdownReport(result) {
    ...
    if (fingerprint) {
        ...
        lines.push(`|---------|-------|`);
        if (fp['projectName'])
            lines.push(`| Project Name | ${fp['projectName']} |`);
        if (fp['appType'])
            lines.push(`| App Type | ${fp['appType']} |`);
        ...
        if (fp['uiFramework'])
            lines.push(`| UI Framework | ${fp['uiFramework']} |`);
        if (fp['backendFramework'])
            lines.push(`| Backend Framework | ${fp['backendFramework']} |`);
        if (fp['testTools'] && Array.isArray(fp['testTools']))
    ...
    if (result.config.uiAnalysis)
        lines.push('| ui-live-basic |');
    lines.push('| report |');
    lines.push('');
...
export function generateScorecardReport(scorecard) {
    return JSON.stringify(scorecard, null, 2);
}
export function writeReports(runPath, result) {
...
export function writePlaceholderReports(runPath) {
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function computeVerdictFromScorecard(scorecard) {
    if (scorecard.overall < 40)
        return 'NO_GO';
    if (scorecard.categories.security < 50)
        return 'NO_GO';
