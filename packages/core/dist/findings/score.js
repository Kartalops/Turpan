 ...
 */
export function calculateScorecard(findings) {
    ...
    const overall = Math.round((build_health * 1.0 +
        test_health * 1.0 +
        code_quality * 0.8 +
        security * 1.0 +
        ui_runtime * 0.6 +
...
function countByCategory(findings) {
    ...
    for (const f of findings) {
        if (!counts[f.category])
            counts[f.category] = { critical: 0, high: 0, medium: 0, low: 0 };
        if (f.severity !== 'info')
 ...
 */
function categoryScore(category, counts) {
    const entry = counts[category];
    if (!entry)
        return 100;
    const weight = CATEGORY_WEIGHTS[category] ?? 0.5;
    const deductions = severityWeight('critical') * entry.critical * 2.0 +
 ...
 */
function releaseScore(findings) {
...
export function countByCategorySimple(findings) {
    const counts = {};
    for (const f of findings)
        counts[f.category] = (counts[f.category] ?? 0) + 1;
    return counts;
}
export function computeVerdict(scorecard, findings) {
    const critical = findings.filter(f => f.severity === 'critical').length;
    const high = findings.filter(f => f.severity === 'high').length;
    if (critical > 0)
        return 'NO_GO';
    if (scorecard.security < 50)
        return 'NO_GO';
    if (scorecard.overall < 40)
        return 'NO_GO';
    if (critical === 0 && high === 0 && scorecard.overall >= 75)
        return 'GO';
