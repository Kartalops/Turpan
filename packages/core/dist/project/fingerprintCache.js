 ...
 */
export async function buildCacheKey(projectRoot) {
    const { readFileSync, existsSync, statSync } = await import('fs');
    const { join } = await import('path');
    const { createHash } = await import('crypto');
    const keyParts = [projectRoot];
 ...
 */
export async function getCachedFingerprint(projectRoot) {
    ...
    if (entry.pid !== process.pid)
        return undefined; // different process
    return entry.fingerprint;
}
/**
 * Store a fingerprint in the cache for the given project root.
 */
export async function cacheFingerprint(projectRoot, fingerprint) {
    const key = await buildCacheKey(projectRoot);
 ...
 */
export function getFingerprintCacheStats() {
    return {
        size: cache.size,
...
//# sourceMappingURL=fingerprintCache.js.map