...
export class NodeBackendRuntimeAnalyzer {
    id = 'node-backend-runtime';
    name = 'Node.js Backend Runtime Analyzer';
    categories = ['runtime', 'node', 'backend'];
    supports(fp) {
        return (fp.appType === 'node-backend' ||
            ['express', 'fastify', 'fastify', 'nestjs'].includes(fp.backendFramework));
    }
    async run(ctx) {
        const errors = [];
        const findings = [];
    ...
    async detectEntrypoint(projectRoot, fp) {
    ...
    async runStartupCheck(projectRoot, entrypoint) {
        const findings = [];
        const errors = [];
        if (!entrypoint) {
    ...
    async analyzePatterns(projectRoot, entrypoint) {
        const findings = [];
        const fullPath = join(projectRoot, entrypoint);
        try {
    ...
    checkUnhandledRejections(content, file) {
        const findings = [];
    ...
    checkUncaughtExceptions(content, file) {
        const findings = [];
    ...
    checkErrorMiddleware(content, file) {
        const findings = [];
...
//# sourceMappingURL=NodeBackendRuntimeAnalyzer.js.map