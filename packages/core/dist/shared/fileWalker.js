...
export function walkFiles(options) {
    const { cwd, extensions, ignoreDirs = DEFAULT_IGNORED_DIRS, ignoreFiles = DEFAULT_IGNORED_FILES, ignoreGlobs = [], ignorePaths = [], maxDepth = 20, followSymlinks = false, } = options;
    const results = [];
    ...
    function isIgnored(absPath) {
        ...
        for (const igPath of ignorePaths) {
            if (rel === igPath || rel.startsWith(igPath + sep))
                return true;
        }
