    ...
    return {
        ...
        async run(ctx) {
            ...
            try {
                ...
                const hasPyproject = pyFiles.some((f) => f.endsWith('pyproject.toml'));
                const hasRequirements = pyFiles.some((f) => f.endsWith('requirements.txt') || f.endsWith('requirements-dev.txt'));
                ...
                for (const file of pyFiles) {
                    ...
                    if (content.length > 200_000)
                        continue;
                    // Skip test files
                    if (/\btest_|_test\.py\b/.test(file))
                        continue;
                ...
                if (bareExceptHits.length > 0) {
                    findings.push(createFinding({
                        ...
                        explanation: `Found ${bareExceptHits.length} bare 'except:' clause(s) that catch all exceptions ` +
                ...
                if (printHits.length > 5) {
                    findings.push({
                        ...
                        severity: 'low',
                        category: 'maintainability',
                        explanation: 'Use the logging module (logging.info, logging.debug, etc.) instead of print ' +
