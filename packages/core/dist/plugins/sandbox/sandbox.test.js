...
import { validatePluginManifest, isPermissionGranted, allPermissionsGranted, buildSandboxContext, isCommandAllowed, sanitizeCommandOutput, isDangerousCommand, DEFAULT_TRUSTED_PLUGINS, } from './index.js';
...
const MALICIOUS_MANIFEST = {
    ...
    permissions: ['read-project-files', 'network-fetch', 'run-commands'],
};
...
describe('validatePluginManifest', () => {
    ...
    it('rejects manifest missing required id', () => {
        const result = validatePluginManifest({ name: 'x', version: '1.0.0' });
    ...
    it('rejects manifest with unknown permission', () => {
        const result = validatePluginManifest({
            ...VALID_MANIFEST,
            permissions: ['read-project-files', 'invalid-permission'],
        });
        expect(result.valid).toBe(false);
...
describe('isPermissionGranted', () => {
    const granted = ['read-package-metadata', 'run-analysis-only'];
...
describe('allPermissionsGranted', () => {
    it('returns granted=true when all permissions are granted', () => {
        const result = allPermissionsGranted(['read-package-metadata'], ['read-package-metadata', 'run-analysis-only']);
        expect(result.granted).toBe(true);
        expect(result.missing).toHaveLength(0);
    });
    it('returns granted=false with missing permissions listed', () => {
        const result = allPermissionsGranted(['network-fetch', 'run-commands'], ['run-commands']);
...
describe('buildSandboxContext', () => {
    ...
    it('allows reading package.json with read-package-metadata', () => {
        const ctx = buildSandboxContext(projectRoot, new Set(['package.json', 'src']), ['read-package-metadata'], 30_000, Date.now(), 'test-plugin');
        const pkg = ctx.getPackageJson();
        expect(pkg).not.toBeNull();
        expect(pkg?.name).toBe('test');
    });
    it('blocks reading package.json without read-package-metadata', () => {
        const ctx = buildSandboxContext(projectRoot, new Set(['package.json']), [], 30_000, Date.now(), 'test-plugin');
        const pkg = ctx.getPackageJson();
        expect(pkg).toBeNull();
    });
    it('blocks path traversal attempts', () => {
        const ctx = buildSandboxContext(projectRoot, new Set(['src']), ['read-project-files'], 30_000, Date.now(), 'test-plugin');
    ...
    it('blocks reading /etc/passwd (path traversal + system file)', () => {
        const ctx = buildSandboxContext(projectRoot, new Set([]), ['read-project-files'], 30_000, Date.now(), 'test-plugin');
    ...
    it('createFinding always injects pluginId and timestamp', () => {
        const ctx = buildSandboxContext(projectRoot, new Set(), ['read-package-metadata'], 30_000, Date.now(), 'my-plugin');
    ...
    it('timeRemainingMs decreases over time', () => {
        ...
        const ctx = buildSandboxContext(projectRoot, new Set(), [], 10_000, start, 'test');
...
describe('Malicious plugin attack vectors', () => {
    ...
    it('path traversal in readFile is blocked by sandbox context', () => {
        const ctx = buildSandboxContext(projectRoot, new Set(['src']), ['read-project-files'], 30_000, Date.now(), 'malicious');
