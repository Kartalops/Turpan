...
export async function runBuildStage(ctx, options = {}) {
    ...
    for (const cmdName of buildCommands) {
        ...
        if (result.blocked) {
            findings.push(createFinding({
                ...
                evidence: [
                    createCommandEvidence(`npm run ${cmdName}`, result.blockReason ?? '', -1, { label: 'blocked-build' }),
                ],
            }));
            continue;
        }
        if (result.timedOut) {
            findings.push(createFinding({
                ...
                tags: ['build', 'timeout'],
                evidence: [
                    createCommandEvidence(`npm run ${cmdName}`, `Build timed out after ${result.durationMs}ms\n${result.stdout}`, -1, { label: 'build-timeout', exitCode: -1 }),
                ],
            }));
            continue;
        }
        if (result.exitCode !== 0) {
            ...
            findings.push(createFinding({
                ...
                explanation: `The build command exited with a non-zero exit code. ` +
                ...
                tags: ['build', 'failure'],
                evidence: [
                    createCommandEvidence(`npm run ${cmdName}`, errorExcerpt, result.exitCode ?? -1, { label: 'build-failure', exitCode: result.exitCode ?? undefined }),
                    result.logPath
                        ? createEvidence('file', {
                            path: result.logPath,
                            label: 'build-log',
