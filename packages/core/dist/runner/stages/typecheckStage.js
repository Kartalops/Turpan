...
export async function runTypecheckStage(ctx, options = {}) {
    ...
    for (const cmdName of typecheckCommands) {
        if (ctx.signal?.aborted)
            break;
        if (!packageScripts[cmdName] && !hasTsConfig)
            continue;
        let result;
        ...
        if (result.exitCode !== 0) {
            findings.push(createFinding({
                ...
                explanation: `The TypeScript compiler reported type errors. ` +
                ...
                evidence: [
                    createCommandEvidence(cmd, result.stdout || result.stderr || '(no output)', result.exitCode ?? -1, { label: 'typecheck-output', exitCode: result.exitCode ?? undefined }),
                    result.logPath
                        ? createEvidence('file', {
                            path: result.logPath,
                            label: 'typecheck-log',
...
//# sourceMappingURL=typecheckStage.js.map