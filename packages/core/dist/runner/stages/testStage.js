...
export async function runTestStage(ctx, options = {}) {
    ...
    for (const cmdName of testCommands) {
        ...
        if (result.blocked) {
            findings.push(createFinding({
                ...
                evidence: [
                    createCommandEvidence(`npm run ${cmdName}`, result.blockReason ?? '', -1, { label: 'blocked-test' }),
                ],
            }));
            continue;
        }
        ...
        if (result.exitCode !== 0) {
            ...
            findings.push(createFinding({
                ...
                tags: ['test', 'failure'],
                evidence: [
                    createCommandEvidence(`npm run ${cmdName}`, result.stdout || result.stderr || '(no output)', result.exitCode ?? -1, { label: 'test-failure', exitCode: result.exitCode ?? undefined }),
                    result.logPath
                        ? createEvidence('file', {
                            path: result.logPath,
                            label: 'test-log',
