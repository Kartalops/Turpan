...
} from '../../analyzers/dependencies/index.js';
import {
  UnusedFileAnalyzer,
  UnusedExportAnalyzer,
} from '../../analyzers/dead-code/index.js';
import {
  PlaceholderAnalyzer,
  DuplicateCodeAnalyzer,
} from '../../analyzers/placeholders/index.js';
import {
  ComplexityHotspotAnalyzer,
} from '../../analyzers/static-quality/index.js';
import {
  ArchitectureBasicAnalyzer,
} from '../../analyzers/architecture-basic/index.js';

// Register all analyzers (idempotent — safe to call multiple times)
function registerAnalyzers(): void {
  const analyzers = [
...
const HANDLED_IDS: StageId[] = ['static-quality', 'dead-code-basic', 'security-basic'];

/**
 * Run the static quality / dead-code / security analyzers.
 * Acts as a single stage that fans out to multiple analyzers.
 */
export async function runStaticQualityStage(
  ctx: ReviewContext,
  stageId: StageId = 'static-quality'
): Promise<StageResult> {
