...
export async function runInstallCheck(
  ...
  if (!hasNodeModules && !options.installCommand) {
    const finding = createFinding({
      ...
      explanation:
        `The node_modules directory is missing. Turpan cannot run build, test, lint, or typecheck ` +
      ...
      createFinding({
        ...
        evidence: [
          createCommandEvidence(
            ...
            { label: 'blocked-install', exitCode: installResult.exitCode ?? undefined }
          ),
        ],
      })
    );
    ...
    const finding = createFinding({
      ...
      evidence: [
        createCommandEvidence(
          ...
          installResult.stdout,
          installResult.exitCode ?? -1,
          { label: 'install-output', exitCode: installResult.exitCode ?? undefined }
        ),
        createEvidence('command-log', {
          command: installCmd,
