...
const ALL_STAGES: Array<{ id: StageId; label: string; categories: string[] }> = [
...
export function generateReviewPlan(
  ...
  const always: StageId[] = ['project-fingerprint', 'install-check', 'script-detection'];
...
export function formatPlanSummary(plan: ReviewPlan): string {
  const lines: string[] = [];
