...
import { createRunContext, finalizeContext } from '../context/index.js';
...
import type { DependencyAuditResult, DependencyInventory } from '@turpan/dependency-audit';
import { createFinding, confidence, type Finding } from '../findings/Finding.js';
import { createEvidence } from '../findings/Evidence.js';

function ensureRunBaseDir(baseRunPath: string): void {
 ...
 */
function deriveDepAuditLimitations(
  ...
  if (inv.projectType === 'python') {
    ...
    if (!hasUv) {
      limitations.push(
        'Python transitive dependencies from uv.lock / poetry.lock are not parsed — only requirements.txt and pyproject.toml are scanned.',
      );
    }
  }
  ...
  if (audit.mode === 'online') {
    limitations.push(
      'Online mode depends on OSV.dev and npm audit availability. Network failures fall back to the offline vulnerability database.',
    );
  } else {
    limitations.push(
      'Offline mode uses only the bundled vulnerability database. Coverage is conservative — production use should pair with online OSV/Snyk for comprehensive scanning.',
    );
  }
  ...
  if (unknownLicenses > 0) {
    limitations.push(
      `${unknownLicenses} dependencies did not declare a license. Self-reported licenses may be stale or incomplete — verify manually for compliance-critical projects.`,
    );
  }
  ...
  if (audit.vulnerabilities.length === 0 && inv.dependencies.length > 0) {
    limitations.push(
      'No vulnerabilities matched the offline database. This does NOT guarantee the project is vulnerability-free — unknown CVEs may exist.',
    );
  }
...
function convertDepAuditToFindings(auditResult: DependencyAuditResult): Finding[] {
  ...
  for (const vf of auditResult.vulnerabilities) {
    ...
    const sev = vuln.severity as Finding['severity'];
    findings.push(
      createFinding({
        ...
        category: 'security',
        explanation:
          `${vuln.title}.${vuln.cveId ? ` CVE: ${vuln.cveId}` : ''} ${vuln.description}`,
        evidence: [
  ...
  for (const lf of auditResult.licenseFindings) {
    if (lf.risk === 'none') continue;
    const riskSeverity: Finding['severity'] =
      lf.risk === 'high' ? 'high' : lf.risk === 'medium' ? 'medium' : 'low';
    findings.push(
      createFinding({
 ...
 */
export async function runAnalysis(options: OrchestratorOptions): Promise<string> {
  const {
    projectPath,
    deepAnalysis = false,
    uiAnalysis = false,
  ...
  try { ensureRunBaseDir(baseRunPath); } catch { /* ignore */ }
  ...
  // Build stage overrides from skip flags
  type StageId = 'project-fingerprint' | 'install-check' | 'script-detection' | 'build' | 'test' | 'lint' | 'typecheck' | 'static-quality' | 'security-basic' | 'dead-code-basic' | 'ui-live-basic' | 'runtime' | 'report';
  const skippedStages: StageId[] = [];
  if (skipBuild) skippedStages.push('build');
  if (skipTests) skippedStages.push('test');
  if (skipLint) skippedStages.push('lint');
  if (skipTypecheck) skippedStages.push('typecheck');
  if (skipStaticQuality) skippedStages.push('static-quality');
  if (skipSecurity) skippedStages.push('security-basic');
  if (skipDeadCode) skippedStages.push('dead-code-basic');
  if (skipUi) skippedStages.push('ui-live-basic');
  if (skipRuntime) skippedStages.push('runtime');
  ...
  const result = await runReview({
    ...
    stageOverrides: skippedStages.length > 0
      ...
      ] as StageId[]].filter(s => !skippedStages.includes(s)) as StageId[])
      : undefined,
  });

  // ── Dependency Audit (runs after all stages, adds findings to the report) ──
  let dependencyAuditResult: DependencyAuditResult | undefined;
  ...
  let dependencyAuditSection: import('@turpan/report').DependencyAuditSection | undefined;
  if (dependencyAuditResult) {
    const inv = dependencyAuditResult.inventory;
    const sbom = dependencyAuditResult.sbom;
    const daSection: import('@turpan/report').DependencyAuditSection = {
 ...
 */
export async function planAnalysis(projectPath: string, options?: { deepAnalysis?: boolean; uiAnalysis?: boolean }): Promise<void> {
  const config = loadConfig(projectPath);
  const fingerprint = await detectProjectAsync(projectPath);
...
export async function runDoctorCheck(): Promise<{ ok: boolean; checks: Array<{ name: string; ok: boolean; message: string }> }> {
