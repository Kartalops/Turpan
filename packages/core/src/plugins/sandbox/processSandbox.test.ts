...
import type { TrustedPluginManifest } from './types.js';
...
const VALID_MANIFEST: TrustedPluginManifest = {
  id: 'test-plugin',
  name: 'Test Plugin',
  version: '1.0.0',
...
function writePlugin(dir: string, behavior: 'normal' | 'infinite' | 'crash' | 'readSecret' | 'malformedJson' | 'hugeOutput'): void {
  const manifest = `{
  id: 'test-plugin',
  name: 'Test Plugin',
  version: '1.0.0',
  ...
  describe('env secret stripping', () => {
    it('plugin cannot read API keys from parent environment', async () => {
      ...
      try {
        ...
        if (original !== undefined) process.env.SUPER_SECRET_API_KEY = original;
        else delete process.env.SUPER_SECRET_API_KEY;
        rmSync(tmpDir, { recursive: true, force: true });
      }
    }, 10_000);
  });
    ...
    it('rejects plugin requiring ungranted permission before spawning', async () => {
      ...
      try {
        ...
        const result = await runProcessSandboxedPlugin({
          ...
          grantedPermissions: ['read-package-metadata', 'run-analysis-only'],
          fingerprint: makeFp({ projectRoot: tmpDir }),
          manifest: { ...VALID_MANIFEST, permissions: ['network-fetch'] as PluginPermission[] },
        });
        expect(result.success).toBe(false);
