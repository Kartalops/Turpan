...
// ── IPC Types (must mirror PluginProcessSandbox.ts) ───────────────────────────
...
type StdinMessage = InitMessage | RunAnalysisMessage | AbortMessage;
...
const PERMISSION_ALLOWED_EXTENSIONS: Record<string, string[]> = {
...
let currentAllowedPaths: Set<string> = new Set();
...
async function main() {
  ...
  try {
    initMsg = JSON.parse(initLine) as InitMessage;
  } catch {
  ...
  const manifest = initMsg.manifest as { id?: string; version?: string; name?: string; permissions?: string[] };
  ...
  // Check supports()
  try {
    const fp = initMsg.fingerprint as { projectRoot?: string };
    ...
    if (msg.type === 'run-analysis') {
      ...
      try {
        ...
        if (typeof exported.register === 'function') {
          const fakeRegistry = {
            registerAnalyzer: (analyzer: { id: string; run: (ctx: unknown) => Promise<{ findings: unknown[] }> }, pluginId: string) => {
...
function buildSandboxContext() {
  ...
  const isExtensionAllowed = (filePath: string, permission: string): boolean => {
  ...
  return {
    ...
    createFinding(partial: Omit<Finding, 'id' | 'pluginId' | 'detectedAt'>): Finding {
      findingCounter++;
      return {
        ...partial,
...
function isPlugin(value: unknown): value is { manifest: { id: string }; supports: (fp: unknown) => boolean; register: (registry: unknown, ctx: unknown) => void } {
...
function sendResult(callId: string, success: boolean, findings: unknown[] | undefined, error: string | undefined): void {
  sendMessage({
    type: 'result',
    callId,
    success,
