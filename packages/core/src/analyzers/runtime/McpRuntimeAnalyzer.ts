...
export class McpRuntimeAnalyzer implements Analyzer {
  id = 'mcp-runtime';
  ...
  private async detectMcpServerFiles(projectRoot: string): Promise<string[]> {
    ...
    for (const file of allFiles) {
      ...
      try {
        ...
        if (
          /@modelcontextprotocol|from ['"]@modelcontextprotocol|from ['"]@anthropic-ai\/mcpsdk/i.test(content) ||
