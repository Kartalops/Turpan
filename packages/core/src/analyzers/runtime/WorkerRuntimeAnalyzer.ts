...
export class WorkerRuntimeAnalyzer implements Analyzer {
  id = 'worker-runtime';
  ...
  private async detectWorkerPattern(projectRoot: string): Promise<{
    detected: boolean;
    framework: string;
    configFiles: string[];
    taskFiles: string[];
    ...
    const workerFrameworks: Array<{ name: string; indicators: string[] }> = [
    ...
    try {
      ...
      if (/celery|rq|redis-queue/i.test(content)) {
        result.detected = true;
        if (!result.configFiles.length) result.framework = 'celery';
      }
    } catch {
      // not found
    }
  ...
  private async checkRetryAndDLQ(
    projectRoot: string,
    workerInfo: { framework: string; taskFiles: string[] }
  ): Promise<Finding[]> {
    const findings: Finding[] = [];

    if (workerInfo.taskFiles.length === 0) return findings;

    const retryPatterns: Record<string, RegExp[]> = {
      celery: [
        /autoretry_for\s*=\s*\[/,
  ...
  private async checkGracefulShutdown(
    projectRoot: string,
    workerInfo: { framework: string; configFiles: string[]; taskFiles: string[] }
  ): Promise<Finding[]> {
    const findings: Finding[] = [];
  ...
  private async checkHeartbeat(
    ...
    if (workerInfo.framework !== 'celery' && workerInfo.framework !== 'bullmq') return findings;
