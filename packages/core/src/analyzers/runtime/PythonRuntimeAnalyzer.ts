...
import { CommandResult } from '../../runner/CommandResult.js';

const WORLD_WRITABLE_CMD_PATTERNS = [
  'curl http', 'wget http', 'requests.', 'aiohttp.', 'httpx.',
];

export class PythonRuntimeAnalyzer implements Analyzer {
  id = 'python-runtime';
  ...
  private async detectRequirements(projectRoot: string): Promise<Record<string, string>> {
  ...
  private async findPythonFiles(projectRoot: string): Promise<string[]> {
  ...
  private async runToolDiscovery(
    projectRoot: string,
    pythonFiles: string[]
  ): Promise<{ findings: Finding[]; errors: string[]; artifacts: Record<string, boolean> }> {
    const findings: Finding[] = [];
    const errors: string[] = [];
  ...
  private async runPytestChecks(
    ...
    if (!result.blocked && result.exitCode === 0) {
      ...
      const testCount = lines.filter((l: string) => l.includes('::') || l.includes('test_')).length;
      if (testCount === 0) {
        findings.push(
          createFinding({
            id: 'python-no-tests',
  ...
  private async runRuffChecks(
    ...
    if (!result.blocked && result.exitCode !== 0) {
      try {
        const issues = JSON.parse(result.stdout || result.stderr || '[]') as Array<{
          filename: string;
        ...
        if (result.stdout) errors.push(`ruff: ${result.stdout.slice(0, 200)}`);
      }
    }
  ...
  private detectBlockingInAsync(
    ...
    if (!/async\s+def/.test(content)) return findings;
  ...
  private async checkEntrypoints(
    projectRoot: string,
    entrypoints: string[]
  ): Promise<Finding[]> {
    const findings: Finding[] = [];
