...
export class CliRuntimeAnalyzer implements Analyzer {
  id = 'cli-runtime';
  ...
  private async detectEntrypoints(projectRoot: string): Promise<string[]> {
    ...
    try {
      ...
      if (pkg.bin) {
        for (const [cmd, file] of Object.entries(pkg.bin as Record<string, string>)) {
      ...
      if (pkg.scripts) {
        for (const [name, cmd] of Object.entries(pkg.scripts as Record<string, string>)) {
          if (['cli', 'console', 'bin', 'cmd'].some(k => name.toLowerCase().includes(k))) {
            entrypoints.push(cmd as string);
          }
        }
      }
    } catch {
    ...
    try {
      ...
      if (match) {
        ...
        for (const line of lines) {
          ...
          if (cmd && val) entrypoints.push(val.replace(/["' ]/g, '').split(' ')[0]);
        }
      }
    } catch {
      // not found
  ...
  private async checkCommandRegistration(
    projectRoot: string,
    entrypoint: string
  ): Promise<Finding[]> {
    const findings: Finding[] = [];
    ...
    const ext = entrypoint.split('.').pop();
    if (!['js', 'ts', 'mjs', 'py'].includes(ext ?? '')) return findings;
