...
export class ArchitectureBasicAnalyzer implements Analyzer {
  ...
  private async detectCircularImports(projectRoot: string, files: string[]): Promise<ReturnType<typeof createFinding>[]> {
    const findings = [];

    // Build a module graph from import statements
    // Keys are resolved absolute paths (relPath), values are also resolved absolute paths
  ...
  private normalizeImportPath(imp: string): string {
    // Remove ./ prefix
  ...
  private async detectApiDuplication(projectRoot: string, files: string[]): Promise<ReturnType<typeof createFinding>[]> {
    const findings = [];

    // Find all api/client/axios/fetch usages and their base URLs/paths
    const apiUsages = new Map<string, { file: string; line: number; pattern: string }[]>();
  ...
  private async detectScatteredEnvUsage(projectRoot: string, files: string[]): Promise<ReturnType<typeof createFinding>[]> {
    const findings = [];

    // Count process.env / import.meta.env references per file
    const envUsagePerFile = new Map<string, number>();
  ...
  private async detectBusinessLogicInUI(projectRoot: string, files: string[]): Promise<ReturnType<typeof createFinding>[]> {
    const findings = [];
    ...
    for (const file of files) {
      if (!componentExtensions.some(ext => file.endsWith(ext))) continue;
