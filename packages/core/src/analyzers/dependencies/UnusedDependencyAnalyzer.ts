...
import { createCodeEvidence, createEvidence } from '../../findings/Evidence.js';
...
export class UnusedDependencyAnalyzer implements Analyzer {
  ...
  async run(ctx: AnalyzerContext): Promise<AnalyzerResult> {
    ...
    const allDeps = new Map<string, string>(
      Object.entries({ ...pkgJson.dependencies, ...pkgJson.devDependencies, ...pkgJson.peerDependencies })
    );
    ...
    for (const [dep, version] of allDeps) {
      if (ALWAYS_USED.has(dep)) continue; // skip framework staples
  ...
  private isUsedInContent(dep: string, content: string): boolean {
