...
export class UnusedExportAnalyzer implements Analyzer {
  id = 'unused-export';
  ...
  async run(ctx: AnalyzerContext): Promise<AnalyzerResult> {
    ...
    const exportMap = new Map<string, Map<string, number>>(); // file -> symbol -> line
    ...
    for (const file of files) {
      try {
        ...
        if (!exportMap.has(file)) exportMap.set(file, new Map());
        for (const [sym, line] of exports) {
          exportMap.get(file)!.set(sym, line);
        }

        // Extract imports
        if (!importMap.has(file)) importMap.set(file, new Set());
  ...
  private extractExports(content: string): Map<string, number> {
  ...
  private buildGlobalImportSet(importMap: Map<string, Set<string>>, files: string[]): Set<string> {
