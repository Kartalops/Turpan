import type { ModelDescriptor, ModelRoute, ModelRoutingInput, ReviewTaskType } from './types.js';
import { ModelRegistry } from './ModelRegistry.js';

const CHEAP_TASKS = new Set<ReviewTaskType>([
  'repo-mapping',
  'file-classification',
...
export class ModelRouter {
  ...
  private score(model: ModelDescriptor, input: ModelRoutingInput): number {
    ...
    if (input.taskType === 'correctness-review') score += model.capabilities.codingReasoning;
    if (input.taskType === 'architecture-review') score += model.capabilities.architectureReasoning;
    if (input.taskType === 'security-review' || input.taskType === 'finding-verification') {
      score += model.capabilities.securityReasoning;
    }
    if (input.visionRequired || input.browserArtifactsExist) score += model.capabilities.vision ? 80 : -100;
    if (input.riskLevel === 'critical' || input.riskLevel === 'high') score += model.capabilities.reliabilityScore;
    if (input.requiredContextSize > 32000 && model.capabilities.longContext) score += 50;
    if (input.previousModelConfidence !== undefined && input.previousModelConfidence < 70) {
      score += model.family === 'reasoning' ? 35 : 0;
    }
    if (input.latencyPreference === 'low') score += model.capabilities.latencyClass === 'low' ? 25 : -10;
