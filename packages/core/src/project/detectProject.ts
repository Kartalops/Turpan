...
import { readJsonFile, fileExists, listDirectory, getPackageJsonInfo, isGitRepository, getGitInfo } from '@turpan/shared';
...
// ============================================================================
// Backwards Compatibility - Old ProjectInfo interface
// ============================================================================
 ...
 */
export function detectBasicProject(projectPath?: string): ProjectInfo {
 ...
 */
export async function detectProjectAsync(projectRoot: string): Promise<ProjectFingerprint> {
 ...
 */
export function detectProject(projectRoot: string): ProjectFingerprint {
...
function computeFingerprint(projectRoot: string): ProjectFingerprint {
  // Run all detectors
...
function detectLanguages(projectRoot: string, pkg: PackageJson | null): string[] {
...
function detectRuntimeType(languages: string[], pkg: PackageJson | null): RuntimeType {
...
function detectImportantFiles(projectRoot: string, fp: ProjectFingerprint): string[] {
  const files: string[] = [];
  ...
  switch (fp.appType) {
    case 'nextjs':
      if (fileExists(`${projectRoot}/app`)) files.push('app/');
      if (fileExists(`${projectRoot}/pages`)) files.push('pages/');
      if (fileExists(`${projectRoot}/src/app`)) files.push('src/app/');
      if (fileExists(`${projectRoot}/src/pages`)) files.push('src/pages/');
      break;

    case 'vite-react':
      if (fileExists(`${projectRoot}/src/main.tsx`)) files.push('src/main.tsx');
      if (fileExists(`${projectRoot}/src/main.ts`)) files.push('src/main.ts');
      if (fileExists(`${projectRoot}/index.html`)) files.push('index.html');
      break;

    case 'fastapi':
      if (fileExists(`${projectRoot}/main.py`)) files.push('main.py');
      if (fileExists(`${projectRoot}/app.py`)) files.push('app.py');
      break;

    case 'telegram-bot':
      if (fileExists(`${projectRoot}/bot.py`)) files.push('bot.py');
      break;

    case 'chrome-extension':
      if (fileExists(`${projectRoot}/manifest.json`)) files.push('manifest.json');
      if (fileExists(`${projectRoot}/background.js`)) files.push('background.js');
      break;

    case 'mcp-server':
      if (fileExists(`${projectRoot}/src/index.ts`)) files.push('src/index.ts');
      if (fileExists(`${projectRoot}/src/server.ts`)) files.push('src/server.ts');
      break;
  }
...
function detectMissingFiles(fp: ProjectFingerprint): string[] {
 ...
 */
export function formatFingerprintSummary(fp: ProjectFingerprint): string {
  const lines: string[] = [];
  ...
  // Docker
  if (fp.dockerAvailable) lines.push('Docker: available');
  if (fp.dockerComposeAvailable) lines.push('Docker Compose: available');
