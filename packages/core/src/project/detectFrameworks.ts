...
import { readJsonFile, fileExists, listDirectory } from '@turpan/shared';
...
export function detectFrameworks(projectRoot: string): FrameworksResult {
...
function detectAppType(projectRoot: string, deps: Record<string, string>): AppType {
  // Next.js
  if (deps.next) {
    return 'nextjs';
  }
...
function detectUiFramework(deps: Record<string, string>): UIFramework {
...
function detectBackendFramework(deps: Record<string, string>, appType: AppType): BackendFramework {
  // If it's a Next.js app, backend is Next.js
  if (appType === 'nextjs') return 'nextjs';
...
function detectTestTools(deps: Record<string, string>): TestTool[] {
...
function detectDatabases(projectRoot: string, deps: Record<string, string>): DatabaseHint[] {
...
function detectAuth(deps: Record<string, string>): AuthHint[] {
...
function detectDeployment(projectRoot: string): DeploymentHint {
  ...
  const pkg = readJsonFile<{ scripts?: Record<string, string> }>(`${projectRoot}/package.json`);
