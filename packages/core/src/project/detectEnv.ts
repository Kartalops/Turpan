...
export function detectEnv(projectRoot: string): EnvResult {
...
function detectEnvFromCode(projectRoot: string): EnvRequirement[] {
...
function scanDirectoryForEnvVars(
  dirPath: string,
  patterns: RegExp[],
  seenVars: Set<string>
): void {
 ...
 */
export function getEnvSummary(result: EnvResult): string {
  const parts: string[] = [];
