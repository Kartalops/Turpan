export type Intent = 'analyze' | 'review' | 'test' | 'ui' | 'clean' | 'fix' | 'report' | 'exit' | 'run' | 'cleanup-scan' | 'quality' | 'find-unused' | 'detect-fake' | 'unknown' | 'deep_review' | 'quick_review' | 'ui_review' | 'runtime_review' | 'code_quality_review' | 'cleanup_review' | 'security_review' | 'agent_output_audit' | 'fix_safe' | 'patch_only' | 'apply_fix' | 'generate_report' | 'open_report' | 'show_findings' | 'show_scorecard' | 'plugin_review';
...
export type PluginPermission = 'read-project-files' | 'read-package-metadata' | 'run-analysis-only' | 'propose-fixes' | 'ui-scenarios' | 'read-config' | 'network-fetch' | 'run-commands';
