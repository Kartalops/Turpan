...
import { createFinding, confidence, type Finding } from '@turpan/core';
import { createEvidence } from '@turpan/core';

const CONFIDENCE_BASE = 85;

function mapSeverity(rules: { critical?: boolean; high?: boolean; medium?: boolean }): 'critical' | 'high' | 'medium' | 'low' {
  if (rules.critical) return 'critical';
  if (rules.high) return 'high';
  if (rules.medium) return 'medium';
  return 'low';
}

export function mapConsoleErrors(entries: ConsoleEntry[], route: string): Finding[] {
  const findings: Finding[] = [];

  for (const entry of entries) {
    if (!entry.isRuntimeError && entry.type !== 'error') continue;
...
export function mapNetworkErrors(requests: NetworkRequest[], route: string): Finding[] {
  const findings: Finding[] = [];

  for (const req of requests) {
    if (req.status < 400 && !req.failure) continue;
...
export function mapFailedInteractions(results: InteractionResult[], route: string): Finding[] {
  const findings: Finding[] = [];
...
export function mapResponsiveIssues(results: ResponsiveResult[]): Finding[] {
  const findings: Finding[] = [];

  for (const result of results) {
    if (!result.hasHorizontalOverflow) continue;
...
export function mapAccessibilityIssues(results: AccessibilityResult[], route: string): Finding[] {
  const findings: Finding[] = [];
...
export function mapBlankPage(route: string): Finding {
  return {
...
export function mapNoOpButton(buttonText: string, route: string): Finding {
  return {
...
export function determineVerdict(
  ...
  interactionFailures: InteractionResult[],
  canStartServer: boolean
): { verdict: UiVerdict; reason: string } {
