...
export class BillingTestModeScenario implements Scenario {
  ...
  private async reportCheckoutWiring(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
    ...
    for (const sel of checkoutSelectors) {
      try {
        ...
        wiringReports.push(
          `${count}x "${sel}" → ${isFake ? `UNWIRED (${reason})` : 'wired'}`
        );
  ...
  private async testLocalCheckout(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
    ...
    let availableEndpoint: string | null = null;
    ...
    try {
      const res = await ctx.page.evaluate(async (endpoint: string) => {
  ...
  private detectFakeCheckoutSuccess(body: unknown): string | null {
