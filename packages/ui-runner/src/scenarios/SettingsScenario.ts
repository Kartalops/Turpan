...
export class SettingsScenario implements Scenario {
  ...
  private async visitSettings(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<void> {
    const start = Date.now();
  ...
  private async inspectForms(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
    ...
    for (const ft of fieldTypes) {
      try {
        ...
        if (count > 0) foundFields.push(`${count}x ${ft.name}`);
      } catch { /* ignore */ }
    }
  ...
  private async detectNoOpSave(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
  ...
  private async checkDestructiveSettings(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
