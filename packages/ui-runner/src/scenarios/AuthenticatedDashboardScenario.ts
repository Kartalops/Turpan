...
export class AuthenticatedDashboardScenario implements Scenario {
  ...
  private async checkMeaningfulContent(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
  ...
  private async clickSafeNavItems(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
  ...
  private async checkForErrors(ctx: ScenarioContext, steps: ScenarioStep[]): Promise<void> {
    const start = Date.now();

    const consoleErrors = ctx.consoleErrors.filter(e =>
      typeof e === 'string' && !e.includes('favicon') && !e.includes('preload')
    );
