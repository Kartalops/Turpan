...
export class DashboardScenario implements Scenario {
  ...
  private async visitDashboard(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<void> {
    const start = Date.now();
  ...
  private async checkSidebar(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
    ...
    let sidebarSel: string | null = null;
  ...
  private async checkWidgets(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<ScenarioFinding[]> {
    const findings: ScenarioFinding[] = [];
    const start = Date.now();
    ...
    for (const sel of widgetSelectors) {
      try {
        ...
        if (n > widgetCount) widgetCount = n;
      } catch { /* ignore */ }
    }
  ...
  private async checkInteractiveWidgets(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<void> {
    const start = Date.now();
    ...
    for (const sel of interactiveSelectors) {
      try {
        ...
        if (count > 0) interactive.push(`${count}x ${sel}`);
      } catch { /* ignore */ }
    }
  ...
  private async checkUserMenu(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<void> {
    const start = Date.now();
  ...
  private async checkNotifications(ctx: ScenarioContext, steps: ScenarioStep[], screenshots: string[]): Promise<void> {
    const start = Date.now();
