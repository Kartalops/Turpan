...
export class AppServerManager {
  ...
  private devCommand: string | null = null;
   ...
   */
  static deriveDevCommand(
    projectRoot: string,
    appType?: string,
    packageScripts?: Record<string, string>,
    explicitDevCommand?: string
  ): string | null {
    if (explicitDevCommand) return explicitDevCommand;

    // Try package.json dev script
    if (packageScripts?.dev) return 'dev';

    // Try next dev for Next.js
    if (appType === 'nextjs' && packageScripts?.['next:dev']) return 'next:dev';

    // Fallback: look for common dev script names
    const candidates = ['dev', 'dev:server', 'start:dev', 'serve', 'start'];
    for (const cand of candidates) {
      if (packageScripts?.[cand]) return cand;
    }
    ...
    try {
      ...
      if (existsSync(pkgPath)) {
        ...
        if (pkg.scripts?.['next:dev']) return 'next:dev';
      }
    } catch { /* ignore */ }
   ...
   */
  async start(devCommand: string, packageManager: string = 'pnpm'): Promise<AppServerInfo> {
    if (this.proc) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onData = (source: 'stdout' | 'stderr', data: Buffer) => {
  ...
  getStartedAt(): string { return this.startedAt; }
