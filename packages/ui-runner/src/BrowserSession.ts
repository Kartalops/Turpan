...
const ACTIVE_SESSIONS = new Set<BrowserSession>();
let BROWSER_CLEANUP_HOOK_INSTALLED = false;

function installBrowserCleanupHooks(): void {
  if (BROWSER_CLEANUP_HOOK_INSTALLED) return;
  BROWSER_CLEANUP_HOOK_INSTALLED = true;

  const cleanup = () => {
    for (const s of ACTIVE_SESSIONS) {
      try { void s.close(); } catch { /* ignore */ }
    }
  };
...
export class BrowserSession {
  ...
  private openPages = new Set<Page>();
  private config: UiRunnerConfig;
  ...
  getPage(): Page | null { return this._page; }
  getContext(): BrowserContext | null { return this.context; }

  getAvailableViewports(): ViewportConfig[] {
    if (this.config.mobileOnly) return [MOBILE_VIEWPORT];
    if (this.config.desktopOnly) return [DESKTOP_VIEWPORT];
  ...
  async close(): Promise<void> {
      ...
      try {
        await Promise.race([
          ...
          new Promise<void>((_, reject) =>
            setTimeout(() => reject(new Error('context close timeout')), BROWSER_CLOSE_TIMEOUT_MS)
          ),
        ]);
      } catch { /* ignore */ }
      this.context = null;
    ...
    if (this.browser) {
      try {
        ...
        try { await this.browser.close().catch(() => {}); } catch { /* ignore */ }
      }
      this.browser = null;
    }
  }
  ...
  static desktopViewport(): ViewportConfig { return DESKTOP_VIEWPORT; }
  static mobileViewport(): ViewportConfig { return MOBILE_VIEWPORT; }
  static getActiveSessionCount(): number { return ACTIVE_SESSIONS.size; }
}
