...
import type { Scenario, ScenarioContext, ScenarioResult } from './scenarios/Scenario.js';
import type { UiRunnerConfig, UiTestReport, UiTestSummary, DiscoveredRoute, ViewportConfig, ResponsiveResult, AccessibilityResult, TestUserConfig, BillingTestConfig } from './types.js';
...
export class UiTestRunner {
  ...
  private a11yScanner: AccessibilityScanner;
  private responsiveScanner: ResponsiveScanner;

  private baseUrl: string = '';
  private routes: DiscoveredRoute[] = [];
  private allConsoleErrors: ReturnType<ConsoleCollector['getErrors']> = [];
  private allNetworkErrors: ReturnType<NetworkCollector['getErrors']> = [];
  private allInteractionResults: ReturnType<InteractionPlanner['getResults']> = [];
  private responsiveResults: ResponsiveResult[] = [];
  private accessibilityResults: AccessibilityResult[] = [];
  private traceFiles: string[] = [];
  private startedAt: string = '';
  private completedAt: string = '';
  private _scenarioResults: ScenarioResult[] = [];
  private _seedOutput: string = '';
  private _safeRunner: SafeCommandRunner;
  ...
  async run(): Promise<UiTestReport> {
    ...
    const verdictResult = determineVerdict(
      ...
      this.allNetworkErrors,
      this.allInteractionResults,
  ...
  private async startServer(): Promise<string> {
    ...
    const devCmd = AppServerManager.deriveDevCommand(
      this.opts.projectRoot,
      this.opts.fingerprint.appType,
      this.opts.fingerprint.packageScripts
    );
  ...
  private async waitForServerReady(url: string, maxAttempts = 15): Promise<void> {
  ...
  private async discoverAndProbeRoutes(): Promise<DiscoveredRoute[]> {
  ...
  private async runViewportTests(viewport: ViewportConfig): Promise<void> {
  ...
  private async detectNoOpButtons(page: Page, route: string): Promise<void> {
    try {
      ...
      for (const btn of buttons) {
        ...
        if (btn.hasClickHandler) continue; // has JS handler, not a no-op
  ...
  private async runResponsiveChecks(viewports: ViewportConfig[]) {
    if (!this.browser.getContext()) return [];
  ...
  private async runAccessibilityChecks(viewports: ViewportConfig[]) {
    if (!this.browser.getContext()) return [];
    const results = [];
  ...
  private async runScenarios(viewports: ViewportConfig[]): Promise<void> {
    ...
    const scenarios: Scenario[] = scenarioIds
      ? (scenarioIds.map(id => scenarioRegistry.get(id)).filter((s): s is Scenario => s !== undefined))
  ...
  private async runSeedCommand(): Promise<void> {
    ...
    const append = (line: string): void => {
      ...
      try { writeFileSync(seedLogPath, stamped, { flag: 'a' }); } catch { /* ignore */ }
    };
  ...
  private buildScenarioSummary(): import('./types.js').ScenarioSummary | undefined {
    if (this._scenarioResults.length === 0) return undefined;
  ...
  private mapToFindings() {
    const findings: ReturnType<typeof mapConsoleErrors> = [];
  ...
  private detectAppType(): 'nextjs' | 'vite-react' | 'unknown' {
    const at = this.opts.fingerprint.appType;
    if (at === 'nextjs') return 'nextjs';
    if (at === 'vite-react') return 'vite-react';
    return 'unknown';
  }

  private buildSummary(): UiTestSummary {
    return {
  ...
  private async saveArtifacts(report: UiTestReport): Promise<void> {
    ...
    const scenarioIdAliases: Record<string, string> = {
      'auth': 'scenario-auth',
  ...
  private async cleanup(): Promise<void> {
    ...
    try { await this.server.stop(); } catch { /* ignore */ }
  }
}
 ...
 */
export async function runUiTest(opts: RunUiTestOptions): Promise<UiTestReport> {
