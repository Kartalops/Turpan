...
const SAFE_TEST_DATA: Record<string, string> = {
...
export class InteractionPlanner {
  ...
  private results: InteractionResult[] = [];
  private page: Page | null = null;
  private viewport: ViewportConfig | null = null;

  /**
   * Plan interactions for a given page.
   * Analyzes the DOM to determine what interactions are possible.
   */
  async plan(page: Page | null, viewport: ViewportConfig): Promise<InteractionStep[]> {
    if (!page) return [];
    this.page = page;
   ...
   */
  async execute(step: InteractionStep): Promise<InteractionResult> {
    if (!this.page) return { step, success: false, error: 'No page set' };

    const result: InteractionResult = { step, success: false };
  ...
  private async executeClick(selector: string, index?: number): Promise<boolean> {
  ...
  private async isSelectorDestructive(selector: string): Promise<boolean> {
    if (!this.page) return false;
    try {
  ...
  private async isPaymentButton(selector: string): Promise<boolean> {
    if (!this.page) return false;
    try {
      const text = await this.page.locator(selector).first().textContent({ timeout: 3000 });
  ...
  private async planCtaClicks(page: Page): Promise<InteractionStep[]> {
  ...
  private async planDropdownOpens(page: Page): Promise<InteractionStep[]> {
    const steps: InteractionStep[] = [];
  ...
  private async planModalOpens(page: Page): Promise<InteractionStep[]> {
    const steps: InteractionStep[] = [];
  ...
  private async planFormInteractions(page: Page): Promise<InteractionStep[]> {
    const steps: InteractionStep[] = [];
  ...
  getResults(): InteractionResult[] { return [...this.results]; }
