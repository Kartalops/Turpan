...
export class AccessibilityScanner {
     ...
     */
    async scan(page, viewport) {
        const issues = [];
        // Collect all issues in parallel
        const [imgIssues, buttonIssues, inputIssues, headingIssues, ariaIssues,] = await Promise.all([
    ...
    async checkImages(page) {
        ...
        try {
            ...
            for (const img of imgs) {
                if (img.ariaHidden === 'true')
                    continue; // intentionally hidden
    ...
    async checkButtons(page) {
        const issues = [];
        try {
            const buttons = await page.$$eval('button, [role="button"]', (btns) => btns.map((btn) => ({
        ...
        try {
            ...
            for (const input of inputs) {
                if (!input.visible)
                    continue;
                // Skip hidden inputs
                if (['hidden', 'submit', 'button', 'reset', 'image', 'checkbox', 'radio'].includes(input.type))
                    continue;
    ...
    async checkHeadings(page) {
        const issues = [];
        try {
            const headings = await page.$$eval('h1, h2, h3, h4, h5, h6', (els) => els.map((el, i) => ({
    ...
    async checkAria(page) {
        ...
        try {
            ...
            const badAria = await page.$$eval('[role="button"][aria-disabled="true"]', (els) => els.map((el) => ({
                tag: el.tagName.toLowerCase(),
