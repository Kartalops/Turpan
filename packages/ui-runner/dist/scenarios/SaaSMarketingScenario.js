...
export class SaaSMarketingScenario {
    id = SCENARIO_ID;
    name = SCENARIO_NAME;
    riskLevel = 'safe';
    supports(fp, routes) {
    ...
    async run(ctx) {
        ...
        try {
            ...
            steps.push(step1);
            if (step1.screenshot)
                screenshots.push(step1.screenshot);
            if (!ctx.page.url().includes('login') && !ctx.page.url().includes('signin')) {
                ...
                steps.push(step2);
                if (step2.screenshot)
                    screenshots.push(step2.screenshot);
                // Step 3: Check primary CTA
                const step3 = await this.checkPrimaryCta(ctx);
                steps.push(step3);
                if (step3.screenshot)
                    screenshots.push(step3.screenshot);
                ...
                screenshots.push(...steps.slice(-3).map(s => s.screenshot).filter(Boolean));
                // Step 6: Scroll test
                const step6 = await this.scrollPage(ctx);
                steps.push(step6);
                if (step6.screenshot)
                    screenshots.push(step6.screenshot);
            }
        }
        catch (err) {
            steps.push({
    ...
    async navigateToSecondaryPages(ctx, steps, screenshots) {
        const pages = [
...
//# sourceMappingURL=SaaSMarketingScenario.js.map