...
export class AuthenticatedAuthScenario {
    id = SCENARIO_ID;
    name = SCENARIO_NAME;
    riskLevel = 'low';
    supports(fp, routes) {
    ...
    async visitLoginPage(ctx, steps, screenshots, loginPath) {
        const start = Date.now();
        try {
    ...
    async fillCredentials(ctx, steps, screenshots, email, password, willSubmit) {
        const start = Date.now();
    ...
    async submitLogin(ctx, steps, screenshots) {
        const findings = [];
        const start = Date.now();
        // Find submit button
    ...
    async verifyRedirect(ctx, steps, screenshots, dashboardPath) {
        const start = Date.now();
    ...
    saveAuthStateMetadata(ctx, steps, screenshots) {
...
//# sourceMappingURL=AuthenticatedAuthScenario.js.map