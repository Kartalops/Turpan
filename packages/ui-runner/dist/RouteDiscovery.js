...
export async function discoverRoutes(options) {
    ...
    for (const route of linkedRoutes)
        candidateRoutes.add(route);
    for (const route of SAAS_ROUTES)
        candidateRoutes.add(route);
    const routes = [];
    const seen = new Set();
 ...
 */
export async function probeRoutes(baseUrl, routes, timeoutMs = 5000) {
...
// ---------------------------------------------------------------------------
async function discoverFromFiles(projectRoot, appType) {
    if (appType === 'nextjs')
        return discoverNextRoutes(projectRoot);
    if (appType === 'vite-react')
        return discoverViteRoutes(projectRoot);
    return [];
}
...
async function walkDir(dir, prefix, routes, opts = {}) {
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (name === 'api' || name === '_next' || name.startsWith('.'))
                continue;
            if (name === 'layout' || name === 'template' || name === 'loading' || name === 'error' || name === 'not-found')
                continue;
...
async function discoverViteRoutes(projectRoot) {
    const routes = new Set();
