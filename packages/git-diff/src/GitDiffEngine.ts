...
import type {
  ChangedFile, ChangeType, DiffHunk, DiffLine, DiffLineType,
  DiffStats, FileOwnership, FileOwnershipHint,
  ChangedRoute, ChangedApi, ChangedComponent,
  GitDiffResult, DiffRiskLevel, DiffReviewRecommendation, DiffFinding,
} from './types.js';
...
function execGit(cwd: string, args: string[]): string {
  try {
    return execFileSync('git', args, { cwd, encoding: 'utf-8', timeout: 30_000 }) as string;
  } catch (err) {
...
function isGitRepo(cwd: string): boolean {
  try {
    execFileSync('git', ['rev-parse', '--git-dir'], { cwd, encoding: 'utf-8', timeout: 5_000 });
    return true;
  } catch { return false; }
}

function refExists(cwd: string, ref: string): boolean {
  try { execGit(cwd, ['rev-parse', '--verify', '--quiet', ref]); return true; } catch { return false; }
}
...
function parseStatusFlag(flag: string): ChangeType {
  return ({ A: 'added', M: 'modified', D: 'deleted', R: 'renamed', C: 'copied' } as Record<string, ChangeType>)[flag[0]] ?? 'modified';
}
...
const OWNERSHIP_PATTERNS: Array<{ pattern: RegExp; ownership: FileOwnership; confidence: number }> = [
...
function detectOwnership(filePath: string): FileOwnershipHint {
  const name = filePath.toLowerCase();
  let best: FileOwnershipHint = { file: filePath, ownership: 'unknown', confidence: 40 };
...
const CRITICAL_PATTERNS = [
  /\bpassword\s*=/i, /\bsecret\s*=/i, /\bapi[_-]?key\s*=/i,
  /\beval\s*\(/i, /\bexec\s*\(/i, /\bchild_process\b/i,
  /process\.env\.[A-Z]/, /\.env(?:\.local|\.development)?/i,
  /auth bypass/i, /SQL injection/i, /hard[_-]?coded[_-]?credential/i,
  /\badmin\s*:\s*true\b/i, /\broot\s*:\s*true\b/i,
  /--allow-unauthenticated/i, /skip.*auth/i,
];

const HIGH_RISK_PATTERNS = [
  /dangerouslySetInnerHTML\b/, /\beval\s*\(/i, /shell\s*:\s*true/i,
  /appendFile\(|writeFile\(/i,
];

function assessRisk(files: ChangedFile[], hunks: DiffHunk[]): DiffRiskLevel {
  const reasons: string[] = [];
  const riskyFiles: string[] = [];

  const secretFiles = files.filter(f =>
    /\bsecret|\bpassword|\bcredential|\benv|\bauth/i.test(f.path) &&
  ...
  for (const hunk of hunks) {
    for (const line of hunk.lines) {
      ...
      for (const p of CRITICAL_PATTERNS) { if (p.test(c)) { reasons.push(`Critical: ${p.source}`); if (!riskyFiles.includes(hunk.filePath)) riskyFiles.push(hunk.filePath); } }
      for (const p of HIGH_RISK_PATTERNS) { if (p.test(c)) { reasons.push(`High-risk: ${p.source}`); if (!riskyFiles.includes(hunk.filePath)) riskyFiles.push(hunk.filePath); } }
    }
  }

  const deletedAuth = files.filter(f =>
    (f.path.includes('auth') || f.path.includes('permission')) && f.changeType === 'deleted');
  if (deletedAuth.length > 0) { reasons.push('Auth/permission files deleted'); riskyFiles.push(...deletedAuth.map(f => f.path)); }

  const total = files.reduce((s, f) => s + f.linesAdded + f.linesDeleted, 0);
  if (total > 2000) reasons.push(`Large diff: ${total} line changes`);

  let level: DiffRiskLevel['level'] = 'low';
...
export function isPathAffectedByDiff(diff: GitDiffResult, filePath: string): boolean {
  return diff.files.some(f =>
    f.path === filePath || f.oldPath === filePath ||
    filePath.endsWith(f.path) || f.path.endsWith(filePath));
}

export function computeDiffRecommendation(
  diff: GitDiffResult,
  additionalFindings?: Array<{ severity: 'critical' | 'high' | 'medium' | 'low' | 'info'; id: string; title: string }>
  ...
  // Merge additional findings from diff-scoped analyzers
  const introducedFindings: Record<string, number> = { critical: 0, high: 0, medium: 0, low: 0, info: 0 };
  ...
  // Risky feature without tests: > 3 non-test, non-doc files changed AND no test files changed
  ...
  // Decision rules:
  // block_merge  — critical introduced findings > 0 OR any critical risk in diff
  // request_changes — high introduced findings > 0 OR risky feature without tests OR medium risk with no test coverage
  // approve      — no critical/high AND (tests pass OR no risky feature changes)
  let decision: DiffReviewRecommendation['decision'] = 'approve';
  if (findings.some(f => f.severity === 'critical') || diff.riskLevel.level === 'critical') {
    decision = 'block_merge';
  } else if (
    introducedFindings.high > 0 ||
    riskyFeatureNoTest ||
    (diff.riskLevel.level === 'medium' && !hasTestChange)
  ) {
    decision = 'request_changes';
  }

  // Confidence:
  // high  — < 30 files AND no refError
  // medium — 30-100 files AND no refError
  // low   — refError present OR > 100 files
  let confidence: DiffReviewRecommendation['confidence'] = 'high';
  if (diff.refError || diff.stats.totalFiles > 100) confidence = 'low';
  else if (diff.stats.totalFiles >= 30) confidence = 'medium';

  // Build category summary from findings
...
export class GitDiffEngine {
  ...
  getDiff(baseRef: string, targetRef: string): GitDiffResult {
    ...
    if (baseExists && targetExists) {
      try { numstatOutput = execGit(root, ['diff', '--numstat', `${baseRef}..${targetRef}`]); }
      catch { refError = `Could not diff ${baseRef}..${targetRef}`; }
    ...
    if (baseExists && targetExists && !refError) {
      try { hunks.push(...this.parseHunks(execGit(root, ['diff', '-U0', `${baseRef}..${targetRef}`]))); } catch { /* ignore */ }
    }

    const stats: DiffStats = {
      totalFiles: files.length, filesAdded: files.filter(f => f.changeType === 'added').length,
      filesModified: files.filter(f => f.changeType === 'modified').length, filesDeleted: files.filter(f => f.changeType === 'deleted').length,
      filesRenamed: files.filter(f => f.changeType === 'renamed').length,
      totalLinesAdded: files.reduce((s, f) => s + f.linesAdded, 0), totalLinesDeleted: files.reduce((s, f) => s + f.linesDeleted, 0),
      totalAdditions: files.reduce((s, f) => s + f.linesAdded, 0), totalDeletions: files.reduce((s, f) => s + f.linesDeleted, 0),
    };

    const changedRoutes: ChangedRoute[] = files
      .filter(f => /\/(app|pages|src\/app|src\/pages)\//.test(f.path) || /\/routes\/|\/router\/|\/endpoints\//.test(f.path.toLowerCase()))
      .map(f => ({ route: '/' + f.path.replace(/^.*\/(app|pages|src\/app|src\/pages)\//, '').replace(/\.(tsx?|jsx?)$/, '').replace(/\[([^\]]+)\]/g, ':$1').replace(/\/+/g, '/'), file: f.path, changeType: f.changeType }));

    const changedApis: ChangedApi[] = files
      .filter(f => /\/api\/|\/rest\/|\/graphql\/|handler\.ts$|handler\.js$/.test(f.path))
      .map(f => { const seg = f.path.split('/api/').pop()?.split('/rest/').pop()?.split('/graphql/').pop(); return { path: '/' + (seg ?? f.path), file: f.path, changeType: f.changeType }; });

    const changedComponents: ChangedComponent[] = files
      .map(f => { const m = f.path.match(/\/([A-Z][a-zA-Z0-9_-]+)\.(tsx?|jsx?)$/); return m ? { name: m[1], file: f.path, changeType: f.changeType } : null; })
      .filter(Boolean) as ChangedComponent[];

    let hasWorkingTreeChanges = false;
    try { hasWorkingTreeChanges = execGit(root, ['status', '--porcelain']).trim().length > 0; } catch { /* ignore */ }
  ...
  isAffected(diff: GitDiffResult, filePath: string): boolean { return isPathAffectedByDiff(diff, filePath); }
  deriveRecommendation(diff: GitDiffResult): DiffReviewRecommendation { return computeDiffRecommendation(diff); }
  ...
  getAvailableRefs() {
    const root = getRepoRoot(this.cwd);
    let current = 'HEAD';
    try { current = execGit(root, ['branch', '--show-current']).trim(); } catch { /* ignore */ }
    let branches: string[] = [];
    try { branches = execGit(root, ['branch', '--format=%(refname:short)']).trim().split('\n').filter(Boolean); } catch { /* ignore */ }
    let tags: string[] = [];
    try { tags = execGit(root, ['tag', '--list']).trim().split('\n').filter(Boolean); } catch { /* ignore */ }
  ...
  private parseHunks(diffOutput: string): DiffHunk[] {
    ...
    for (const rawLine of lines) {
      ...
      if (rawLine.startsWith('---')) { currentHunkIndex = 0; continue; }
      ...
      if (currentHunk) {
        const type: DiffLineType = rawLine.startsWith('+') ? 'added' : rawLine.startsWith('-') ? 'deleted' : 'context';
