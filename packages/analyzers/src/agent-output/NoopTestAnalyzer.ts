...
const NOOP_PATTERNS: NoopPattern[] = [
  // Only truthy check
  {
...
export interface NoopTestOptions {
  projectRoot: string;
  testFiles: string[]; // files that are tests
}

export function analyzeNoopTests(opts: NoopTestOptions): AgentOutputIssue[] {
 ...
 */
export function findTestFiles(projectRoot: string, extensions = ['ts', 'tsx', 'js', 'jsx']): string[] {
  const tests: string[] = [];
