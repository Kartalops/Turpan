...
const README_PATTERNS: ClaimPattern[] = [
  {
    category: 'auth',
...
const IMPLEMENTATION_PATTERNS: Record<string, string[]> = {
...
export function analyzeReadmeMismatch(opts: ReadmeMismatchOptions): AgentOutputIssue[] {
  const { projectRoot, taskCapabilities } = opts;
  const issues: AgentOutputIssue[] = [];

  const readmePath = join(projectRoot, 'README.md');
  if (!existsSync(readmePath)) return issues;
...
function scanForStrings(projectRoot: string, strings: string[]): boolean {
