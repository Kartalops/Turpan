 ...
 */
export function analyzeUnwiredFeatures(opts: UnwiredFeatureOptions): AgentOutputIssue[] {
...
function isApiRoute(file: string, relPath: string): boolean {
  return /\/(api|routes)\//.test(relPath) || /\.route\.(ts|js)$/.test(file) || /\/route\.(ts|js)$/.test(relPath);
}

function isComponent(file: string, relPath: string): boolean {
  return /\/components?\//.test(relPath) || /\/ui\//.test(relPath) || /\/widgets?\//.test(relPath);
}

function isHandler(file: string, relPath: string): boolean {
...
function extractComponentName(file: string, _relPath: string): string {
  const name = basename(file, extname(file));
  return name.replace(/^[A-Z]/, (c: string) => c.toLowerCase()); // PascalCase to camelCase
}

function extractHandlerName(relPath: string): string {
  const name = basename(relPath);
  return name.replace(/\.(ts|tsx|js|jsx)$/, '').replace(/^[A-Z]/, (c: string) => c.toLowerCase());
}

function getReferences(
  routeKey: string,
  _importGraph: Map<string, Set<string>>,
  _files: string[],
  projectRoot: string
): string[] {
...
function findImportOfComponent(componentName: string, files: string[], projectRoot: string): string[] {
  const refs: string[] = [];
  for (const file of files) {
    if (file === projectRoot) continue;
    try {
...
function checkNextJsUnwired(projectRoot: string): AgentOutputIssue[] {
  const issues: AgentOutputIssue[] = [];
  ...
  try {
    ...
    for (const pageFile of pageFiles) {
      ...
      const camelName = pageName.replace(/^[A-Z]/, (c: string) => c.toLowerCase());
