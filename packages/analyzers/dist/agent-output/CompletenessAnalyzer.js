...
export async function runAgentOutputAudit(opts) {
    ...
    const completion = computeCompletionScore(task.capabilities, implementation, allIssues, testFiles.length, sourceFiles.length);
    // 7. Build evidence files list
    const evidenceFiles = [...new Set(allIssues.flatMap(i => i.evidence.filter(e => e.path).map(e => e.path)))];
...
function detectMissingCapabilities(task, implementation) {
    const issues = [];
...
function severityForCategory(category) {
    const critical = ['auth', 'billing', 'security'];
    const high = ['database', 'backend-endpoints', 'api-design', 'tests'];
    const medium = ['ui-pages', 'dashboard', 'workers', 'mcp-server', 'cli', 'integrations'];
    if (critical.includes(category))
        return 'critical';
...
// ── Completion Score ─────────────────────────────────────────────────────────
function computeCompletionScore(capabilities, implementation, issues, testCount, sourceCount) {
    ...
    const overall = Math.round(requestedFeatureCoverage * COVERAGE_WEIGHT +
...
function computeTestRelevance(testCount, issues) {
    if (testCount === 0)
        return 0;
...
function computeRuntimeValidation(issues) {
    const criticalFakes = issues.filter(i => i.kind === 'fake-implementation' && (i.severity === 'critical' || i.severity === 'high')).length;
    return Math.max(0, Math.min(100, 100 - criticalFakes * 20));
}
// ── Helpers ──────────────────────────────────────────────────────────────────
function computeRecommendation(completion, issues) {
    const criticalCount = issues.filter(i => i.severity === 'critical').length;
    const highCount = issues.filter(i => i.severity === 'high').length;
    const missingCritical = completion.missingCapabilities.some(c => ['auth', 'billing', 'security', 'database'].includes(c.category));
    if (criticalCount > 0 || missingCritical)
        return 'MAJOR_REWORK';
    if (highCount > 2 || completion.overall < 50)
        return 'NOT_READY';
    if (highCount > 0 || completion.overall < 75)
        return 'READY_WITH_LIMITATIONS';
    return 'READY';
}
function computeConfidenceLevel(completion, issues) {
    const highIssues = issues.filter(i => i.confidence >= 80).length;
    const lowIssues = issues.filter(i => i.confidence < 60).length;
    if (highIssues > lowIssues && completion.overall > 50)
        return 'high';
    if (lowIssues > highIssues || completion.overall < 30)
        return 'low';
    return 'medium';
}
function buildSummary(task, completion, issues) {
    ...
    if (missingCount > 0)
        parts.push(`${missingCount} capabilities are missing.`);
    if (fakeCount > 0)
        parts.push(`${fakeCount} fake/shallow implementations detected.`);
    if (noopCount > 0)
        parts.push(`${noopCount} no-op tests found.`);
...
//# sourceMappingURL=CompletenessAnalyzer.js.map