...
function walkDir(dir, maxDepth = 5, depth = 0) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.name === 'node_modules' || entry.name === '.git' || entry.name === 'dist' || entry.name === 'build' || entry.name === '.next' || entry.name === '.turpan' || entry.name === 'coverage')
                continue;
 ...
 */
export function mapImplementation(projectRoot, task, opts) {
...
function isSignificantFile(relPath) {
    const skipPatterns = [
        /node_modules/,
        /\.git\//,
        /dist\//,
...
function classifyFile(relPath, fullPath, task) {
    const capabilitySet = new Set(task.capabilities.map(c => c.category));
...
function extractEndpoint(relPath) {
 ...
 */
export function discoverApiRoutes(projectRoot) {
...
//# sourceMappingURL=ImplementationMapper.js.map