...
export const DEFAULT_FIX_POLICY: FixPolicy = {
 ...
 */
export function policyForMode(mode: FixMode, overrides: Partial<FixPolicy> = {}): FixPolicy {
  const base: FixPolicy = { ...DEFAULT_FIX_POLICY, ...overrides };
 ...
 */
export function mergePolicy(base: FixPolicy, overrides: Partial<FixPolicy>): FixPolicy {
  return {
    ...base,
    ...overrides,
 ...
 */
export function isAutoApplicable(policy: FixPolicy, category: FixCategory): boolean {
  if (policy.blockedCategories.includes(category)) return false;
 ...
 */
export function isModeAllowed(policy: FixPolicy, mode: FixMode): boolean {
 ...
 */
export function requiresConfirmation(policy: FixPolicy, mode: FixMode, category: FixCategory): boolean {
  if (mode === 'interactive') return true; // always confirm in interactive
  if (mode === 'apply' && category === 'manual') return true; // manual category needs confirm in apply mode
  if (isAutoApplicable(policy, category)) return false;
  return true;
}
 ...
 */
export function validatePolicy(policy: FixPolicy): string[] {
  const errors: string[] = [];
  ...
  const unknownModes = policy.allowedModes.filter(
    m => !['report-only', 'patch-only', 'apply', 'interactive', 'auto-safe'].includes(m)
  );
  if (unknownModes.length > 0) {
    errors.push(`Unknown fix modes: ${unknownModes.join(', ')}`);
  }

  const unknownCategories = policy.blockedCategories.filter(
    c => !['safe', 'unsafe', 'manual'].includes(c)
  );
