...
import type { FixRunResult, FixCandidate } from './types.js';
import type { FixPlan } from './FixPlanner.js';
import { formatPatchHeader } from './PatchGenerator.js';

// ─── Fix Plan Report ─────────────────────────────────────────────────────────

function renderCandidate(c: FixCandidate): string {
...
export function renderFixPlanReport(plan: FixPlan): string {
  const lines: string[] = [];
  ...
  if (plan.applied.length > 0) {
    ...
    for (const c of plan.applied) {
      lines.push(renderCandidate(c as FixCandidate));
      lines.push('');
    }
  }
  ...
  if (plan.rejected.length > 0) {
    ...
    lines.push(`|-----------|---------|--------|`);
    for (const c of plan.rejected as FixCandidate[]) {
  ...
  if (plan.deferred.length > 0) {
    ...
    lines.push('');
    for (const c of plan.deferred as FixCandidate[]) {
  ...
  if (plan.requiredChecks.length === 0) {
    ...
    lines.push(plan.requiredChecks.map((c: string) => `- [ ] ${c}`).join('\n'));
  }
  lines.push('');
...
export function renderFixResultReport(result: FixRunResult): string {
  const lines: string[] = [];
