...
export interface FixPlan {
  ...
  applied: FixCandidate[];    // candidates that will/would be applied
  rejected: FixCandidate[];  // candidates rejected by policy
  deferred: FixCandidate[];  // awaiting interactive confirmation
 ...
 */
export function buildFixPlan(config: FixPlannerConfig): FixPlan {
  const {
    projectRoot,
    fixMode,
    findings,
  ...
  const rejected: FixCandidate[] = [];
  const deferred: FixCandidate[] = [];
  ...
  for (const candidate of candidates) {
    ...
    if (
      candidate.finding.category === 'dependency' &&
 ...
 */
export function buildFixRunResult(
  plan: FixPlan,
  ...
  const applied: FixItemResult[] = plan.applied.map(c => ({
    candidateId: c.id,
  ...
  const rejected: FixItemResult[] = plan.rejected.map(c =>
    rejectCandidate(c, 'policy-blocked')
  );

  const deferred: FixItemResult[] = plan.deferred.map(c => ({
    candidateId: c.id,
    findingId: c.finding.id,
    decision: 'deferred',
  }));
 ...
 */
export function summarizePlan(plan: FixPlan): string {
  const lines: string[] = [];
