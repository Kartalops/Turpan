...
export function selectImpactedTests(projectRoot, candidate, reproductions = []) {
    const tests = [];
...
function nearestPackageRoot(projectRoot, file) {
...
function findSiblingTest(projectRoot, file) {
    const absoluteDir = join(projectRoot, dirname(file));
    if (!existsSync(absoluteDir))
        return undefined;
    const stem = basename(file, extname(file));
    const candidates = readdirSync(absoluteDir).filter((entry) => entry === `${stem}.test.ts` ||
        entry === `${stem}.test.tsx` ||
        entry === `${stem}.spec.ts` ||
        entry === `${stem}.spec.tsx`);
