export function assessRegressionTest(diff) {
    const reasons = [];
    if (!diff?.trim()) {
    ...
    if (!hasTest)
        reasons.push('no test case added');
    if (!hasAssertion)
        reasons.push('no assertion added');
    if (skipped)
        reasons.push('test is skipped');
    if (alwaysTrue)
        reasons.push('contains always-true assertion');
    if (swallowed)
        reasons.push('swallows exceptions');
    if (mocksImplementation)
        reasons.push('appears to mock implementation under test');
    return { meaningful: reasons.length === 0, reasons };
}
//# sourceMappingURL=RegressionTestGuard.js.map