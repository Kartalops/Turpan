 ...
 */
function computeLineDiff(original, replacement) {
 ...
 */
export function generateSinglePatch(candidate) {
 ...
 */
export function generatePatch(candidates) {
    ...
    for (const filePath of sortedFiles) {
        const fileCandidates = byFile.get(filePath).sort((a, b) => a.startLine - b.startLine);
        for (const candidate of fileCandidates) {
            // Check for file deletion marker (endLine >= 999999)
            if (candidate.replacementSnippet === '' &&
 ...
 */
export function formatPatchHeader(mode, projectRoot) {
